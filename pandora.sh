...
# Run the podman command with the provided architecture and bind mount
# USB bind mount for ADB access or other USB devices theoretically.
podman run --rm -it --privileged -v /dev/bus/usb:/dev/bus/usb -h pandora -v "${CURRENT_DIR}/projects:/root/projects" localhost/${ARCHITECTURE}-${VARIANT}-${TYPE}-$IMAGE_TAG bash
